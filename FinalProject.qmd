...
MoviesJoined %>%
  ...
  geom_line(linewidth = 1.2) +    # Draw the lines with thicker width
...
CumulativeProfitData <- MoviesJoined %>%
  ...
  group_by(Company) %>%
  mutate(CumulativeProfit = cumsum(YearlyProfit)) # Compute cumulative profit
...
ggplot(
  ...
  theme(
    ...
    legend.title = element_text(size = 14, face ="bold"),# legend size and bold
